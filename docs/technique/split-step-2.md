...
1. Opponent is about to strike the ball
2. Player split-steps
3. The nervous system loads elastic energy into the legs (like a compressed spring)
4. The legs are primed to move in any direction
5. Directional cues from the opponent's contact resolve which direction is taken
6. The stored elastic energy releases into that direction
...
This is [Embodied Cognition](embodied-cognition.md) at the level of the lower limbs: the body is engaged in cognitive preparation before the brain has resolved the problem.

## Timing as Expertise

The timing of the split step is itself a learned, calibrated skill. It must coincide with the opponent's contact moment — not before (the energy dissipates), not after (the spring cannot load). Getting this timing right is a form of [Predictive Processing](predictive-processing.md): the player must predict when the opponent will contact the ball in order to synchronize the split step.
