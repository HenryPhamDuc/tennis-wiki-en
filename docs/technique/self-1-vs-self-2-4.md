...
This is the psychological architecture that the [Kinetic Chain](../biomechanics/kinetic-chain.md)'s physical demands require to operate correctly.
...
**The result:** "Antagonistic tension" — opposing muscles contract simultaneously. This creates [Petit Bras](petit-bras.md) (short-arming the ball), where the stroke loses its elastic power and fluidity. The [Stretch-Shortening Cycle](stretch-shortening-cycle.md) cannot fire because the muscles are already activated by Self 1's instructions; there is no eccentric stretch to load.

This is the same mechanism as [Arming](arming.md) — both are the physical result of Self 1 overriding Self 2's movement programming. Arming is the biomechanical description; Petit Bras is the Inner Game description. They describe the same phenomenon from different perspectives.
...
The [Dorsal Attention Network Dominance](dorsal-attention-network-dominance.md) article (Alcaraz vault) describes the neural architecture that keeps a player in Self 2 under pressure. The DAN is the operating system that maintains implicit processing; Self 2 is the functional description of what that implicit system *is*. The [Implicit Decision Trees](implicit-decision-trees.md) are the specific programmed content of Self 2's repertoire.
...
The handbook's four tools for quieting Self 1:
1. **[Bounce-Hit Technique](../psychology/bounce-hit-technique.md)** — gives Self 1 a simple, specific task (saying the words), occupying it so it cannot wander into analysis or judgment
2. **[Non-Judgmental Observation](../psychology/non-judgmental-observation.md)** — trains the evaluation response from judgment to data
3. **[Quiet Eye](quiet-eye.md)** — narrows Self 1's focus to a sensory detail (the ball's seams), preventing analytical interference
