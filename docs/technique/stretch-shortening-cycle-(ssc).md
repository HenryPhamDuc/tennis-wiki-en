...
In tennis, the SSC is the primary mechanism that converts [Triple Flexion](triple-flexion.md) loading into [Triple Extension](triple-extension.md) power. It explains why a player who "bounces" into a stroke hits harder than one who simply swings from a static position.
...
The muscle-tendon unit lengthens under active tension. In tennis:
- The Achilles tendon is stretched during [Dorsiflexion](dorsiflexion.md) on landing
- The quadriceps and hamstrings load during knee flexion (120°–130°)
- The glutes and posterior chain load during hip hinge
...
The brief period between the end of eccentric loading and the start of the concentric release. This is the [Amortization Phase](amortization-phase.md) — the most critical timing window in the SSC. If the amortization phase is too long (>150ms), stored elastic energy dissipates as heat and is lost. The player then relies on slower, voluntary concentric muscle contraction to move.

### 3. Concentric Phase (Release)
The muscle-tendon unit shortens explosively, driving [Triple Extension](triple-extension.md). At the ankle, this manifests as [Plantar Flexion](plantar-flexion.md) — the Achilles tendon releases its stored energy as the gastrocnemius fires.
...
The [Split-Step Calibration](split-step-calibration.md) is essentially the setup for a lower-body SSC event — landing correctly *is* loading the spring.

---

## The 150ms Rule

Elastic energy dissipates as heat if the amortization phase exceeds approximately **150ms**. This is why:
- Elite split-step ground contact times target **< 150–200ms**
- The "heavy" or "slow" feeling after landing on stiff legs is literal energy loss — the spring was compressed but could not release in time
- Players who pause or "settle" between landing and first step are operating outside the SSC window
