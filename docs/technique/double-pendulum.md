...
This is a parametric energy transfer governed by [Conservation of Angular Momentum](conservation-of-angular-momentum.md), and it is the reason elite players can generate extreme racket-head speed from what appears to be a relaxed, flowing motion.
...
When a player "muscles" the shot by pulling the arm forward throughout:
- The upper arm never reaches zero velocity at contact
- The transfer is partial — some `L` stays in the arm instead of reaching the racket
- The racket-head speed is limited to the arm's own velocity rather than being amplified by the transfer

When the player releases correctly:
- The upper arm decelerates (fired by [Internal Shoulder Rotation](internal-shoulder-rotation.md) then braked)
- All `L` transfers to the racket
- The racket head achieves angular velocity *greater* than any single-segment push could produce
...
- Large `r` from axis to racket head
- High [Tangential Velocity](tangential-velocity.md) (`v = ωr`) at given `ω`
- Higher centrifugal load on shoulder stabilisers
- See [Straight-Arm vs Double-Bend](../players/straight-arm-vs-double-bend.md)
