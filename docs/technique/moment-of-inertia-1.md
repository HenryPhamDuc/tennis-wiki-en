...
From [Conservation of Angular Momentum](conservation-of-angular-momentum.md): `L = Iω = constant`. This means `I` and `ω` are inversely proportional — decrease `I` and `ω` spikes automatically, without any additional muscular contraction.
...
The [Non-Hitting Arm](non-hitting-arm.md) begins the stroke extended outward (high `r`, high `I`). As the forward swing initiates, it is pulled violently into the torso (low `r`, low `I`). Because `L` is conserved, the torso's `ω` surges. This "dumps" the momentum directly into the hitting shoulder, producing the extreme racket-head speeds seen in Alcaraz and Sinner.
...
At contact, players like Alcaraz and Federer fully extend the hitting arm, maximising `r`. At the moment of maximum `ω` (inherited from the preceding segment's deceleration), a large `r` maximises the tangential velocity at the tip (`v = ωr`). This is the [Straight-Arm vs Double-Bend](../players/straight-arm-vs-double-bend.md) trade-off: Sinner maximises `ω` by keeping `I` low; Alcaraz maximises `r` at the moment `ω` peaks.
...
The [Double Pendulum](double-pendulum.md) system in the arm also operates through `I` manipulation. As the forearm decelerates (its `I` effectively "locks"), the racket's angular velocity must spike to conserve the momentum being transferred from the arm. This parametric transfer is why "letting the racket drop" into the slot before accelerating produces more speed than consciously forcing the swing forward.
