---
title: "Approach Shot First Protocol"
language: en
...
The approach shot determines the quality of the passing shot the volleyer must handle. An excellent approach shot:
- Forces the baseline player to strike from a wide or deep position
- Arrives with pace and/or spin that limits the passer's options
- Creates a time constraint that reduces the passer's swing speed
- Produces a passing attempt from a compromised position
...
An average approach shot:
- Gives the baseline player time to set up
- Arrives at a comfortable height and distance
- Allows the passer to execute their best swing
- Produces a fast, accurate passing attempt
...
**Direction:** An approach down the line (closing the angle) is more effective than cross-court (which opens angles for the opponent). The [45-Degree Rule](45-degree-rule.md) for net interception only applies if the approach shot has positioned the volleyer correctly — which requires a down-the-line or central approach.
