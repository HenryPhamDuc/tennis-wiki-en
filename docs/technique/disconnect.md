...
The **Disconnect** is a [core fault](core-leaks.md) defined by the loss of coordinated tension between torso segments during the forward swing. Rather than firing in a precise sequential order, the segments of the [Kinetic Chain](../biomechanics/kinetic-chain.md) fire simultaneously, out of sequence, or with gaps in tension — breaking down the chain's ability to amplify force as it travels from proximal to distal.

It is one of the four [Core Leaks](core-leaks.md), and the one most related to the timing and coordination dimension of [Proximal-to-Distal Sequencing](proximal-to-distal-sequencing.md) rather than its spatial dimension.

---

## Core Mechanism

The [Kinetic Chain](../biomechanics/kinetic-chain.md) is not merely a sequence of body segments — it is a system of linked segments that must maintain **coordinated tension** throughout the swing. Each segment must:
...
3. Decelerate in a timed way that transfers momentum upward (see [Braking Failure](../biomechanics/braking-failure.md))
...
The Disconnect differs from the other [Core Leaks](core-leaks.md) in that it is primarily a **timing fault** rather than a spatial or directional fault:

| Fault | Primary dimension of failure |
|---|---|
| [Bucket Leak](bucket-leak.md) | Spatial — wrong pelvic orientation (tilt) |
| [Sway Fault](sway-fault.md) | Spatial — wrong pelvic movement (lateral vs. rotational) |
| [Braking Failure](../biomechanics/braking-failure.md) | Timing — proximal segments don't decelerate correctly |
