...
- The **[Non-Hitting Arm](non-hitting-arm.md)** is pulled violently into the torso during the forward swing, reducing total `I` and forcing `ω` of the trunk to surge.
...
| Phase | Arm Position | Radius (r) | Moment of Inertia (I) | Angular Velocity (ω) |
|---|---|---|---|---|
| **Preparation** | Extended out | High | High | Low (stability phase) |
| **Acceleration** | Tucked in | Low | Low | **High (explosion phase)** |

The arm begins wide (storing potential energy in the [X-Factor](x-factor.md) stretch), then tucks inward as the forward swing initiates. This "figure skater tuck" is the trigger that converts stored elastic energy into peak angular velocity at the distal end.
...
Where `θ` is the [X-Factor](x-factor.md) angle (shoulder rotation minus hip rotation). The faster the X-Factor uncoils — and the more the NHA tucks simultaneously — the higher `ω` becomes. Both variables are multiplied together in the conservation equation, making their timing relationship critical.
...
Failing to maintain the [X-Factor](x-factor.md) separation — rotating as a unit — can cut potential power by half, because the sequential velocity transfer never occurs.

---

## The VOR Constraint on Conservation

A hidden limiting factor: the brain monitors head angular acceleration via the vestibular system. If the head rotates too quickly (as it does when a player tracks the ball with their eyes *and* rotates their shoulders simultaneously), the [Vestibulo-Ocular Reflex](vestibulo-ocular-reflex.md) fires a reflexive postural correction — which the CNS implements by **throttling the trunk's angular momentum**.
