...
In tennis, Dorsiflexion is the ankle's specific contribution to [Triple Flexion](triple-flexion.md). It is what separates a productive, spring-loaded landing from a flat-footed energy-dead one.
...
1. **Pre-stretch of the Achilles tendon**: The tendon is placed under eccentric tension, acting like a compressed spring. This is the entry point for the [Stretch-Shortening Cycle (SSC)](stretch-shortening-cycle-(ssc).md) in the ankle.
...
In the [Split-Step Calibration](split-step-calibration.md), Dorsiflexion must be maintained through the landing. If the shin angle collapses backward and the heel makes first contact, the spring pre-load is lost and the [Amortization Phase](amortization-phase.md) becomes dead weight rather than stored energy.
...
Failing to maintain ankle Dorsiflexion during the split-step landing creates what the sources call the **"Flat-Foot State."** The player lands on their heels, which:
- Bypasses the Achilles pre-stretch
- Triggers a slower, muscle-driven recovery rather than a tendon-elastic one
- Imposes a neurological latency penalty of approximately **150ms** on first-step initiation
...
Dorsiflexion is the *loading* action; [Plantar Flexion](plantar-flexion.md) is the *release* action. During the [Triple Extension](triple-extension.md) push-off (the serve launch or lateral explosion), the ankle reverses from Dorsiflexion into Plantar Flexion — the gastrocnemius fires, delivering the terminal "snap" into the court that adds the final 10–15% of upward velocity to the center of gravity.
