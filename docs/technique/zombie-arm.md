---
title: "Zombie Arm"
language: en
vault: Tennis Wiki-English
category: "technique"
created: 2026-06-21
---

# Zombie Arm

Zombie Arm is a failure mode where the hitting arm operates independently of the body's rotation and kinetic chain, leading to weak, injury-prone strokes.

It is one of the most common flaws in recreational tennis, resulting from a misunderstanding of how power is generated.

---

## Core Mechanism / How It Works
When the [Dantian & Spinal Axis](dantian-&-spinal-axis.md) is unstable or the [Kinetic Chain](../biomechanics/kinetic-chain.md) is broken, the player attempts to compensate by swinging the arm aggressively. The arm disconnects from the torso, swinging ahead of the hips and shoulders. This completely eliminates [Separation Angle](separation-angle.md) and the elastic whip effect. Power becomes entirely muscular rather than biomechanical.

## Failure Modes / Common Errors / When It Breaks
| Failure Mode | Cause | Consequence |
|---|---|---|
| Arm Domination | Initiating the swing with the hand rather than the hips | Weak shots, rapid fatigue, and high risk of shoulder/elbow injury |
| Early Release | The arm crosses the plane of the body before the hips open | Loss of [Lag Mechanics](lag-mechanics.md) and rotational power |

## Training / Application / Implementation
To cure Zombie Arm, players must relearn how to keep the arm connected to the torso's rotation. This involves drills where the arm is kept relatively passive while the hips and core initiate the forward movement, ensuring the arm only fires at the very end of the sequence.

## Related Concepts
- [5-Year Tennis Mastery System](5-year-tennis-mastery-system.md)
- [Separation Angle](separation-angle.md)
- [Kinetic Chain](../biomechanics/kinetic-chain.md)

---

> 🌐 **[Read in Tiếng Việt](https://henryphamduc.github.io/tennis/tennis-wiki/)** — Vietnamese version of this wiki

