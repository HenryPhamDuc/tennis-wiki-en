...
1. **Eccentric Loading**: As the knees bend, muscles stretch like a rubber band, storing elastic energy (see [Stretch-Shortening Cycle](stretch-shortening-cycle.md))
...
Elite lateral movement is overwhelmingly a medial-lateral GRF challenge, not a linear one. The player must produce massive force along the side-to-side vector to:
- Brake on the outside foot at wide balls
- Violently reverse direction toward centre
...
GRF application requires pre-loading: the athlete's feet must be planted and their eccentric loading phase already initiated *before* the incoming ball bounces on their side. If the feet are flat at contact, the kinetic chain is broken at the first link. This demands the [Split Step](split-step.md) be timed precisely — landing exactly as the opponent makes contact — so the stored GRF elastic energy can drive the first explosive step.
