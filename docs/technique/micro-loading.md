...
Full strength sessions (85–95% 1RM, high volume, 45–90 phút):
- Create significant muscle damage → inflammatory response.
- Require **2–3 ngày recovery** để neuromuscular function return to baseline.
- Nếu thực hiện trong tournament week → player đang competing với partially-recovered neuromuscular system.
- Kết quả: Serves 10 km/h slower, split-step timing 50ms slower — "despite trying hard."
...
**Key**: Fast, explosive execution even at 60–70% load — not slow strength work. Neural stimulus comes from **velocity of movement**, not just load magnitude (xem [Type IIb Fast-Twitch Optimisation](type-iib-fast-twitch-optimisation.md)).

---

## HRV Override Rule

Even with micro-loading protocol, **HRV determines whether the session happens**:
...
The schedule is the default. The HRV is the governor. (Xem [HRV-Guided Training](../system/hrv-guided-training.md).)
